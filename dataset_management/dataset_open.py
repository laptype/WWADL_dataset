...
def process_and_save_dataset_v2(root_path, time_len, time_step, modality_list=None, output_dir='./output', name='',
                                batch_size=100, target_len = 2048, receivers_to_keep = None, new_mapping=None, id2action = None, target_volunteer_id=1):
    # 创建输出目录
    ...
    file_records = dataset.generate_file_list(volunteer_ids=[0, 9, 15])
    # train_list, test_list = dataset.split_data_by_volunteer(file_records, target_volunteer_id=target_volunteer_id)
    print(file_records)
    train_list = file_records
    test_list = file_records
    # 导出文件列表为 CSV
...
if __name__ == "__main__":
    ...
    #     target_volunteer_id=target_volunteer_id
    # )
    ...
    # 目标文件夹路径
    target_folder = '/root/shared-nvme/WWADL_open'

    # 创建目标文件夹（如果不存在的话）
    if not os.path.exists(target_folder):
        os.makedirs(target_folder)

    # 为 imu, wifi, AirPodsPro 文件夹创建目录
    imu_folder = os.path.join(target_folder, 'imu')
    wifi_folder = os.path.join(target_folder, 'wifi')
    airpods_folder = os.path.join(target_folder, 'AirPodsPro')

    # 创建这些目录（如果不存在的话）
    os.makedirs(imu_folder, exist_ok=True)
    os.makedirs(wifi_folder, exist_ok=True)
    os.makedirs(airpods_folder, exist_ok=True)

    # 遍历每个记录并复制文件
    for record in file_records:
        # 获取文件路径
        imu_path = record['imu_path']
        wifi_path = record['wifi_path']
        airpods_path = record['airpods_path']
        
        # 确定文件名和目标路径
        imu_target_path = os.path.join(imu_folder, os.path.basename(imu_path))
        wifi_target_path = os.path.join(wifi_folder, os.path.basename(wifi_path))
        airpods_target_path = os.path.join(airpods_folder, os.path.basename(airpods_path))
        
        # 复制文件
        shutil.copy(imu_path, imu_target_path)
        shutil.copy(wifi_path, wifi_target_path)
        shutil.copy(airpods_path, airpods_target_path)

        # 如果需要更新记录中的路径，可以做类似操作
        record['imu_path'] = os.path.join('imu', os.path.basename(imu_path))
        record['wifi_path'] = os.path.join('wifi', os.path.basename(wifi_path))
        record['airpods_path'] = os.path.join('AirPodsPro', os.path.basename(airpods_path))
        # 将更新后的 file_records 转换为 DataFrame
    df = pd.DataFrame(file_records)

    # 保存为 CSV 文件
    csv_file_path = os.path.join(target_folder, 'file_records.csv')
