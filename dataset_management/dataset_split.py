...
def process_and_save_dataset(root_path, time_len, time_step, modality_list=None, output_dir='./output', name='', batch_size=100, target_len = 2048):
    """
    ...
    train_data, train_labels = train_dataset.segment_data(time_len, time_step, target_len = target_len)

    train_h5_path = os.path.join(output_dir, 'train_data.h5')
    train_json_path = os.path.join(output_dir, 'train_label.json')
    save_data_in_batches(train_data, train_labels, batch_size, train_h5_path, train_json_path)
...
def generate_info_json(output_dir, train_data, train_labels, test_data, test_labels, modality_list, time_len, time_step, segment_info = None):
    ...
    info = {
        ...
        "modality_list": modality_list,
        "train_data": {
            "num_samples": {modality: data.shape[0] for modality, data in train_data.items()},
            "data_shape": {modality: data.shape[1:] for modality, data in train_data.items()}
        },
        "test_data": {
            "num_samples": {modality: data.shape[0] for modality, data in test_data.items()},
            "data_shape": {modality: data.shape[1:] for modality, data in test_data.items()}
        },
        "labels": {
            "train": {modality: len(train_labels[modality]) for modality in train_labels},
            "test": {modality: len(test_labels[modality]) for modality in test_labels}
